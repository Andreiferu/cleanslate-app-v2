      ...
      <main className="max-w-7xl mx-auto px-3 sm:px-6 py-4 sm:py-8">
        {activeTab === 'dashboard' && (
          <div className="space-y-6 sm:space-y-8 animate-fade-in">
            ...
            {data.insights && data.insights.length > 0 && (
              <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-gray-200 p-4 sm:p-6">
                ...
                <div className="text-center mb-4 sm:mb-6">
                  ...
                  <p className="text-sm sm:text-base text-gray-600 font-medium">Subscription Efficiency</p>
                </div>
                <div className="space-y-3 sm:space-y-4">
                  <div className="flex justify-between items-center p-2 sm:p-3 bg-green-50 rounded-lg">
                    <span className="text-xs sm:text-sm font-medium">Active subscriptions</span>
                    <span className="font-bold text-green-600 text-sm sm:text-base">{analytics.activeSubscriptions}/{analytics.totalSubscriptions}</span>
                  </div>
                  <div className="flex justify-between items-center p-2 sm:p-3 bg-orange-50 rounded-lg">
                    <span className="text-xs sm:text-sm font-medium">Monthly savings opportunity</span>
                    <span className="font-bold text-orange-600 text-sm sm:text-base">${analytics.potentialSavings.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between items-center p-2 sm:p-3 bg-purple-50 rounded-lg">
                    <span className="text-xs sm:text-sm font-medium">Email cleanup progress</span>
                  ...
                  <div className="flex justify-between items-center p-2 sm:p-3 bg-blue-50 rounded-lg">
                    <span className="text-xs sm:text-sm font-medium">Annual billing savings</span>
                    <span className="font-bold text-blue-600 text-sm sm:text-base">${analytics.yearlyDiscount.toFixed(2)}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-gray-200 p-4 sm:p-6">
                <h3 className="text-base sm:text-lg font-semibold mb-4 sm:mb-6">Spending by Category</h3>
                <div className="space-y-3 sm:space-y-4">
                  {Object.entries(
                    data.subscriptions
                        ...
                        <div key={category} className="space-y-2">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center space-x-2 sm:space-x-3 min-w-0 flex-1">
                              <div className={`w-3 h-3 sm:w-4 sm:h-4 rounded-full flex-shrink-0`} style={{backgroundColor: colors[index % colors.length]}}></div>
                              <span className="font-medium text-sm sm:text-base truncate">{category}</span>
                            </div>
                            <div className="text-right flex-shrink-0 ml-2">
                              <p className="font-bold text-sm sm:text-base">${amount.toFixed(2)}</p>
              ...
              {/* Mobile Optimized Financial Projections */}
              <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-gray-200 p-4 sm:p-6">
                ...
                <div className="space-y-4 sm:space-y-6">
                  <div className="p-3 sm:p-4 bg-red-50 rounded-xl border border-red-200">
                    <h4 className="font-medium text-red-800 mb-1 sm:mb-2 text-sm sm:text-base">Current Path</h4>
                    <p className="text-2xl sm:text-3xl font-bold text-red-600">${(analytics.monthlySpend * 12).toFixed(2)}</p>
                    <p className="text-xs sm:text-sm text-red-600 font-medium">Annual subscription cost</p>
                  </div>
                  <div className="p-3 sm:p-4 bg-green-50 rounded-xl border border-green-200">
                    <h4 className="font-medium text-green-800 mb-1 sm:mb-2 text-sm sm:text-base">Optimized Path</h4>
                    <p className="text-2xl sm:text-3xl font-bold text-green-600">
                      ${((analytics.monthlySpend - analytics.potentialSavings) * 12).toFixed(2)}
                    </p>
                    <p className="text-xs sm:text-sm text-green-600 font-medium">With recommended changes</p>
                  </div>
                  <div className="p-4 sm:p-6 bg-gradient-to-r from-green-50 via-emerald-50 to-green-50 rounded-xl border-2 border-green-200">
                    <div className="text-center">
                      <p className="text-green-800 font-bold text-sm sm:text-lg mb-1 sm:mb-2">
              ...
              <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-gray-200 p-4 sm:p-6">
                <h3 className="text-base sm:text-lg font-semibold mb-4 sm:mb-6">Savings Goal Progress</h3>
                <div className="space-y-4 sm:space-y-6">
                  <div>
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-xs sm:text-sm font-medium text-gray-700">Progress to ${data.user.savingsGoal} goal</span>
                      <span className="text-xs sm:text-sm font-bold text-blue-600">{analytics.progressToGoal.toFixed(1)}%</span>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 sm:gap-4">
                    <div className="p-3 sm:p-4 bg-blue-50 rounded-xl border border-blue-200 text-center">
                      <h4 className="font-semibold text-blue-800 text-xs sm:text-sm">Time Saved</h4>
                    ...
                    <div className="p-3 sm:p-4 bg-purple-50 rounded-xl border border-purple-200 text-center">
                      <h4 className="font-semibold text-purple-800 text-xs sm:text-sm">Subscriptions</h4>
                      <p className="text-lg sm:text-2xl font-bold text-purple-600">{analytics.totalSubscriptions}</p>
                      <p className="text-xs text-purple-600 font-medium">total managed</p>
                    </div>
                  </div>
                  
                  {analytics.progressToGoal >= 100 && (
                    <div className="p-3 sm:p-4 bg-gradient-to-r from-yellow-50 to-orange-50 rounded-xl border-2 border-yellow-200">
                      <div className="text-center">
                        <div className="text-3xl sm:text-4xl mb-2">🎉</div>
                        <h4 className="font-bold text-yellow-800 text-sm sm:text-base">Congratulations!</h4>
                        <p className="text-xs sm:text-sm text-yellow-700">You've reached your savings goal!</p>
                      </div>
                    </div>
                  )}
                </div>
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-gray-200 p-4 sm:p-6">
              <h3 className="text-base sm:text-lg font-semibold mb-4 sm:mb-6">Executive Summary</h3>
              ...
              <div className="mt-4 sm:mt-6 p-3 sm:p-4 bg-gradient-to-r from-indigo-50 via-purple-50 to-pink-50 rounded-xl border border-indigo-200">
                <h4 className="font-bold text-indigo-800 mb-2 text-sm sm:text-base">💡 Key Insights</h4>
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-gray-200 p-4 sm:p-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                <div className="p-3 sm:p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg border border-blue-200">
                  <h4 className="font-semibold text-blue-800 mb-2 text-sm sm:text-base">💡 Quick Win</h4>
                  <p className="text-xs sm:text-sm text-blue-700">Cancel Adobe Creative Cloud (unused for 3 months) to save $52.99/month instantly.</p>
                </div>
                <div className="p-3 sm:p-4 bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg border border-green-200">
                  <h4 className="font-semibold text-green-800 mb-2 text-sm sm:text-base">📈 Long-term Strategy</h4>
                  <p className="text-xs sm:text-sm text-green-700">Switch 4 subscriptions to annual billing to save $127.45/year in total.</p>
                </div>
                <div className="p-3 sm:p-4 bg-gradient-to-r from-purple-50 to-pink-50 rounded-lg border border-purple-200">
                  <h4 className="font-semibold text-purple-800 mb-2 text-sm sm:text-base">🎯 Goal Focus</h4>
                  <p className="text-xs sm:text-sm text-purple-700">You need to save ${(data.user.savingsGoal - data.user.totalSaved).toFixed(2)} more to reach your goal.</p>
                </div>
                <div className="p-3 sm:p-4 bg-gradient-to-r from-yellow-50 to-orange-50 rounded-lg border border-yellow-200">
                  <h4 className="font-semibold text-yellow-800 mb-2 text-sm sm:text-base">📧 Email Action</h4>
                  <p className="text-xs sm:text-sm text-yellow-700">Unsubscribe from 3 high-volume senders to save 33 minutes/week.</p>
                </div>
              </div>
            </div>
          </div>
...
}3 className="text-lg sm:text-xl font-bold text-gray-900 flex items-center mb-2 sm:mb-0">
          ...
          <div className="space-y-4 sm:space-y-6 animate-fade-in">
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-gray-200 p-4 sm:p-6">
              <div className="mb-4 sm:mb-6">
                ...
                <p className="text-sm sm:text-base text-gray-600">Manage all your subscriptions in one place</p>
              </div>
                
              {/* Mobile Optimized Controls */}
              <div className="space-y-3 sm:space-y-0 sm:flex sm:space-x-3 mb-4 sm:mb-6">
                ...
                <div className="grid grid-cols-2 gap-2 sm:flex sm:space-x-2 sm:gap-0">
                  <select
                    ...
                    className="px-3 sm:px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm sm:text-base"
                  >
              ...
              <div className="grid grid-cols-3 sm:grid-cols-6 gap-2 sm:gap-4">
                <div className="text-center p-2 sm:p-4 bg-gradient-to-br from-green-50 to-emerald-100 rounded-lg sm:rounded-xl border border-green-200">
                  <p className="text-lg sm:text-2xl font-bold text-green-600">{analytics.activeSubscriptions}</p>
                  <p className="text-xs font-medium text-gray-600">Active</p>
                </div>
                <div className="text-center p-2 sm:p-4 bg-gradient-to-br from-yellow-50 to-orange-100 rounded-lg sm:rounded-xl border border-yellow-200">
                  <p className="text-lg sm:text-2xl font-bold text-yellow-600">{analytics.unusedSubscriptions}</p>
                  <p className="text-xs font-medium text-gray-600">Unused</p>
                </div>
                <div className="text-center p-2 sm:p-4 bg-gradient-to-br from-red-50 to-red-100 rounded-lg sm:rounded-xl border border-red-200">
                  <p className="text-lg sm:text-2xl font-bold text-red-600">{analytics.forgottenSubscriptions}</p>
                  <p className="text-xs font-medium text-gray-600">Forgotten</p>
                </div>
                <div className="text-center p-2 sm:p-4 bg-gradient-to-br from-blue-50 to-blue-100 rounded-lg sm:rounded-xl border border-blue-200">
                  <p className="text-lg sm:text-2xl font-bold text-blue-600">{analytics.pausedSubscriptions}</p>
                  <p className="text-xs font-medium text-gray-600">Paused</p>
                </div>
                <div className="text-center p-2 sm:p-4 bg-gradient-to-br from-gray-50 to-gray-100 rounded-lg sm:rounded-xl border border-gray-200">
                  <p className="text-lg sm:text-2xl font-bold text-gray-600">{analytics.cancelledSubscriptions}</p>
                  <p className="text-xs font-medium text-gray-600">Cancelled</p>
                </div>
                <div className="text-center p-2 sm:p-4 bg-gradient-to-br from-purple-50 to-purple-100 rounded-lg sm:rounded-xl border border-purple-200">
                  <p className="text-sm sm:text-xl font-bold text-purple-600">${analytics.potentialSavings.toFixed(0)}</p>
            ...
            {filteredSubscriptions.length === 0 && (
              <div className="text-center py-8 sm:py-12">
                ...
                <h3 className="text-lg sm:text-xl font-semibold text-gray-900 mb-2">No subscriptions found</h3>
        ...
        {activeTab === 'emails' && (
          <div className="space-y-4 sm:space-y-6 animate-fade-in">
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-gray-200 p-4 sm:p-6">
              <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-4 sm:mb-6">Email Cleanup & Management</h2>
              
              {/* Mobile Optimized Email Stats */}
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 mb-6 sm:mb-8">
                <div className="bg-gradient-to-r from-red-50 to-orange-50 p-3 sm:p-6 rounded-lg sm:rounded-xl border border-red-200">
                  <h4 className="font-semibold text-red-800 mb-1 sm:mb-2 text-xs sm:text-sm">Weekly Volume</h4>
                  <p className="text-2xl sm:text-3xl font-bold text-red-600">{analytics.emailsPerWeek}</p>
                  <p className="text-xs sm:text-sm text-red-600 font-medium">emails</p>
                </div>
                <div className="bg-gradient-to-r from-green-50 to-emerald-50 p-3 sm:p-6 rounded-lg sm:rounded-xl border border-green-200">
                  <h4 className="font-semibold text-green-800 mb-1 sm:mb-2 text-xs sm:text-sm">Cleaned Up</h4>
                  <p className="text-2xl sm:text-3xl font-bold text-green-600">{data.emails.filter(e => e.unsubscribed).length}</p>
                  <p className="text-xs sm:text-sm text-green-600 font-medium">senders</p>
                </div>
                <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-3 sm:p-6 rounded-lg sm:rounded-xl border border-blue-200">
                  <h4 className="font-semibold text-blue-800 mb-1 sm:mb-2 text-xs sm:text-sm">Time Saved</h4>
                  <p className="text-2xl sm:text-3xl font-bold text-blue-600">{Math.round(data.emails.filter(e => e.unsubscribed).length * 10)}</p>
                  <p className="text-xs sm:text-sm text-blue-600 font-medium">min/week</p>
                </div>
                <div className="bg-gradient-to-r from-purple-50 to-pink-50 p-3 sm:p-6 rounded-lg sm:rounded-xl border border-purple-200">
                  <h4 className="font-semibold text-purple-800 mb-1 sm:mb-2 text-xs sm:text-sm">Progress</h4>
                  <p className="text-2xl sm:text-3xl font-bold text-purple-600">
                    {Math.round((data.emails.filter(e => e.unsubscribed).length / data.emails.length) * 100)}%
                  </p>
                  <p className="text-xs sm:text-sm text-purple-600 font-medium">complete</p>
                </div>
              </div>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-2 mb-2">
                          <h4 className="font-bold text-gray-900 text-sm sm:text-lg">{email.sender}</h4>
        ...
        {activeTab === 'analytics' && (
          <div className="space-y-6 sm:space-y-8 animate-fade-in">
            {/* Mobile Optimized Analytics Header */}
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-gray-200 p-4 sm:p-6">
              <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">Advanced Analytics</h2>
              <p className="text-sm sm:text-base text-gray-600">Comprehensive insights into your digital spending patterns and optimization opportunities.</p>
            </div>

            {/* Mobile Optimized Analytics Grid */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
              {/* Mobile Optimized Digital Health Score */}
              <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-gray-200 p-4 sm:p-6">
                <h// pages/index.jsx - Mobile Optimized CleanSlate Application
...
const SubscriptionCard = ({ subscription, onCancel, onPause, onActivate, isCompact = false }) => {
  ...
  if (isCompact) {
    return (
      <div className={`p-3 sm:p-4 rounded-xl border-2 transition-all duration-300 hover:shadow-lg active:scale-95 ${getStatusColor()}`}>
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2 sm:space-x-3 min-w-0 flex-1">
            <span className="text-xl sm:text-2xl flex-shrink-0">{subscription.logo}</span>
            <div className="min-w-0 flex-1">
              <h4 className="font-semibold text-gray-900 text-sm sm:text-base truncate">{subscription.name}</h4>
              <div className="flex items-center space-x-1 sm:space-x-2">
                {getStatusIcon()}
                <span className="text-xs sm:text-sm capitalize text-gray-700">{subscription.status}</span>
              </div>
            </div>
          </div>
          <div className="text-right flex-shrink-0">
            <p className="font-bold text-sm sm:text-lg text-gray-900">${subscription.amount}</p>
